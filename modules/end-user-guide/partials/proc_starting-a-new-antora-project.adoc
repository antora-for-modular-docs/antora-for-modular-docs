...
. Download the https://github.com/themr0c/antora-for-modular-docs/archive/refs/heads/main.zip[Antora for Modular Docs zip archive] and decompress it. The archive contains:
...
. Use the content of this archive to start a new repository.
+
----
...
. To publish the build result for the pull requests, set the `NETLIFY_SITE_ID` environment variable in the remote git repository configuration (settings/secrets/actions). See: link:https://github.com/netlify/actions/tree/master/cli[Netlify GitHub Action CLI documentation]

. Optionally, set `NETLIFY_SITE_ID` environment variable in the remote git repository configuration (settings/secrets/actions), to declare the API site ID. See: link:https://docs.netlify.com/cli/get-started/#link-with-an-environment-variable[Link with an environment variable].
